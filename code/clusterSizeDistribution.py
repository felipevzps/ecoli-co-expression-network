...
def cumulative_fraction_of_genes(data):
    # cumulative distribution fraction: https://www.geeksforgeeks.org/how-to-calculate-and-plot-a-cumulative-distribution-function-with-matplotlib-in-python/
    cdf = np.cumsum(data) / np.sum(data)
    return cdf

def plot_cdf(cluster_sizes, cdf, label, marker, color):
    #plt.step(cluster_sizes, cdf, linestyle='-', linewidth=1, marker=marker, markersize=7, label=label, color=color, markerfacecolor='none', where='mid') 
...
for inflation, marker, color in zip(inflation_values, markers, colors):
    file_name = f'clusterSizeDistribution_I{inflation}.txt'
